...
import {
    HashRouter, Switch, Route, Redirect, RouteComponentProps
...
function RenderRouter(list: MenuItem[]): React.ReactNode {
    return list.map((item: MenuItem) => {
        const { routes, ...currentItem } = item;
        ...
        <Provider store={RootStore}>
            <HashRouter>
                <Switch>
                    ...
                    <Route path="/workBench" key="/workBench" component={(props: RouteComponentProps)=> (<ContentLayout {...props}/>)}/>
                    {RenderRouter(Menu)}
